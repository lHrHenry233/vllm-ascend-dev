...
# ============================================================
# V2f: V2 + 2D LOAD -> TYPE CAST -> 2D STORE (break chain)
# ============================================================
@triton.jit
def kernel_v2f(
    x_ptr, w_ptr, bias_ptr,
    ...
    """V2 + 2D load -> .to(float32).to(bf16) -> 2D store. Break data dep."""
    idx_seq = tl.load(batch_ptr + tl.program_id(0)).to(tl.int64)
    chunk_offset = tl.load(token_chunk_offset_ptr + tl.program_id(0))
    idx_feats = tl.program_id(1) * BLOCK_N + tl.arange(0, BLOCK_N)
    ...
    if chunk_offset == 0:
        ...
        if state_len <= seqlen:
            ...
            # --- TYPE CAST to break data dependency chain ---
            loaded_x = loaded_x.to(tl.float32).to(x_ptr.dtype.element_ty)

            # --- 2D store: write loaded_x to pool ---
            idx_tok_c = tl.arange(0, NP2_STATELEN)
            dest_coord = tl.load(
                cache_indices_ptr + idx_seq * stride_cache_indices
            ).to(tl.int64)
...
# ============================================================
# V2g: V2 + 1D ROW LOOP (replace 2D load->store with row-by-row)
# ============================================================
@triton.jit
def kernel_v2g(
    x_ptr, w_ptr, bias_ptr,
    ...
    """V2 + 1D row loop for state write-back (no 2D tensors at all)."""
    idx_seq = tl.load(batch_ptr + tl.program_id(0)).to(tl.int64)
    chunk_offset = tl.load(token_chunk_offset_ptr + tl.program_id(0))
    idx_feats = tl.program_id(1) * BLOCK_N + tl.arange(0, BLOCK_N)
    ...
    if chunk_offset == 0:
        ...
        if state_len <= seqlen:
            # --- 1D row loop: load from x, store to pool, one row at a time ---
            dest_coord = tl.load(
                cache_indices_ptr + idx_seq * stride_cache_indices
            ).to(tl.int64)
            mask_f2 = idx_feats < dim
...
def run_v2f():
    ...
    kernel_v2f[grid](
        ...
        -1, NP2_STATELEN=4, BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N)
    torch.npu.synchronize()
    print("✅ V2f PASSED: V2 + 2D load -> cast -> 2D store (chain break)")
...
def run_v2g():
    ...
    kernel_v2g[grid](
        ...
        -1, NP2_STATELEN=4, BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N)
    torch.npu.synchronize()
    print("✅ V2g PASSED: V2 + 1D row loop (no 2D tensors)")
...
if __name__ == "__main__":
    tests = [
        ...
        ("V2f: + 2D load -> cast -> 2D store (chain break)", run_v2f),
        ("V2g: + 1D row loop (no 2D tensors)", run_v2g),
    ]
