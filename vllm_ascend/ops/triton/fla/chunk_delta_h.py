...
def chunk_gated_delta_rule_fwd_h(
    ...
    h = k.new_empty(B, NT, H, K, V, dtype=state_dtype if state_dtype is not None else k.dtype)
    h_update = k.new_empty(B, NT, H, K, K)
    final_state = k.new_empty(N, H, K, V, dtype=state_dtype if state_dtype is not None else torch.float32) if output_final_state else None
